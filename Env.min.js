module.exports= function(name, opts) {
    ...
    return new (class {
      ...
      wxToken(){
        const corpid= this.getval('@weixin.corpid');
        const corpsecret= this.getval('@weixin.corpsecret')
        return new Promise(resolve => {
        var options = {  
      ...
      async wxMsg(text,desp,userid='gaoyao'){
        let wxtoken='';
        var date= this.getdata("@weixin.date");   
        var isavailable=date>new Date().getTime()- 2*60 * 60 * 1000;  
        if(isavailable){
          wxtoken= this.getdata("@weixin.token");   
          //console.log(wxtoken);     
        }else{
          wxtoken= await this.wxToken();
          this.setdata(wxtoken,'@weixin.token') 
          this.setdata(new Date().getTime(),'@weixin.date') 
        } 
